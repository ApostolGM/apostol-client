...
export default function Campaign({ user }) {
  ...
  useEffect(() => {
    ...
    socket.on('character_updated', handleCharacterUpdate);
    socket.on('dice_result', handleDiceResult);
    socket.on('chat_message', handleChatMessage);
    socket.on('inventory_updated', handleInventoryUpdated);

    return () => {
      socket.off('character_updated', handleCharacterUpdate);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-wasteland-900 flex items-center justify-center">
        <p className="text-wasteland-300 font-stylized">Загрузка...</p>
      </div>
    );
  }
  ...
  const handleTimeChange = async (dateStr, hours, minutes) => {
    ...
    try {
      await api.updateCampaignTime(id, {
        game_time_date: dateStr,
    ...
    <div className="h-screen bg-wasteland-900 flex flex-col overflow-hidden">
      <header className="bg-wasteland-800 border-b border-wasteland-600 p-2 md:p-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-wasteland-400 hover:text-wasteland-200 text-sm"
          >
            ←
          </button>
          <h1 className="text-base md:text-xl font-stylized text-accent-orange truncate max-w-[120px] md:max-w-none">
            {campaign.title}
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2">
          {isMaster && (
            <button
              ...
              className={`text-xs px-2 py-0.5 rounded ${
                hiddenMode
                  ? 'bg-accent-red text-wasteland-900'
                  : 'bg-wasteland-600 text-wasteland-300'
              }`}
              title={hiddenMode ? 'Скрытые броски' : 'Открытые броски'}
            >
              {hiddenMode ? '🔒' : '👁'}
            </button>
          )}
          <span className="text-wasteland-500 text-xs hidden sm:inline">
            Код: {campaign.invite_code}
          </span>
          ...
          {isMaster ? (
            <div className="flex items-center gap-1">
              ...
              {saveStatus === 'error' && (
                <span className="text-accent-red text-xs">⚠️</span>
              )}
            </div>
          ) : (
      ...
      {error && (
        <div className="bg-accent-red/10 border border-accent-red/30 p-2 text-accent-red text-sm text-center">
          {error}
          <button onClick={loadCampaign} className="ml-2 underline">Повторить</button>
        </div>
      )}
          ...
          {activeTab === 'chat' && (
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
              <div ref={chatRef} className="flex-1 overflow-y-auto p-3 space-y-2 min-h-0">
                {messages.length === 0 && (
                  <p className="text-wasteland-500 text-center mt-8 text-sm">
                    Чат пуст. /r 2d10 + 3 для броска
                  </p>
                )}
                {messages.map((m, i) => (
                  <div
                    ...
                    className={`text-sm ${
                      m.isRoll
                        ? 'bg-wasteland-800/50 p-1.5 rounded border border-wasteland-700'
                        : ''
                    }`}
                  >
              ...
              <form
                ...
                className="p-2 bg-wasteland-800 border-t border-wasteland-600 flex gap-2 flex-shrink-0"
              >
                <input
                ...
                <button
                  type="submit"
                  className="bg-wasteland-600 text-wasteland-100 px-3 py-2 rounded text-sm hover:bg-wasteland-500 transition flex-shrink-0"
                >
                  →
                </button>
              </form>
  ...
  <div key={m.user_id} className="text-sm py-1.5 px-2 rounded mb-1 bg-wasteland-700/50 flex justify-between items-center">
    <div>
      <div className="flex items-center gap-1">
        ...
        <span className="text-wasteland-300">
          {m.user?.username || m.user_id?.substring(0, 8)}
        </span>
      </div>
      {m.character_id && (
        <span className="text-wasteland-500 text-xs">🎭 В игре</span>
      )}
    </div>
    {isMaster && m.user_id !== user.id && m.role !== 'master' && (
      <button
        onClick={() => {
          if (confirm(`Выгнать игрока "${m.user?.username}" из кампании?`)) {
            api.deleteMember(id, m.user_id).then(() => loadCampaign());
          }
        }}
        className="text-accent-red hover:text-red-400 text-xs"
        title="Выгнать"
      >
        🚫
      </button>
