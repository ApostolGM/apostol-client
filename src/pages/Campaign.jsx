...
import CampaignHeader from './campaign/CampaignHeader.jsx';
import CampaignTabs from './campaign/CampaignTabs.jsx';
import ChatSection from './campaign/ChatSection.jsx';
...
export default function Campaign({ user }) {
  ...
  useEffect(() => {
    ...
    socket.on('dice_result', (data) => addMessage({
      user: data.username,
    ...
    socket.on('chat_message', (data) => addMessage({
      ...
      time: new Date(data.created_at).toLocaleTimeString(), isRoll: data.is_roll,
    }));
    ...
    socket.on('death_loan_requested', async (data) => {
      ...
      if (ok) {
        ...
        socket.emit('death_loan_approve', { campaignId: data.campaignId, characterId: data.characterId, count: char?.death_loan_count || 0, rollResult: 20 });
      }
    });
    socket.on('death_loan_approved', () => { addMessage({ user: 'Система', text: '💀 Рассрочка гибели активирована!', time: new Date().toLocaleTimeString() }); refreshCharacter(); });
    socket.on('death_loan_forced', () => { addMessage({ user: 'Система', text: '💀 Мастер активировал провал!', time: new Date().toLocaleTimeString() }); refreshCharacter(); });

    return () => {
      socket.off('character_updated'); socket.off('dice_result'); socket.off('chat_message');
      socket.off('inventory_updated'); socket.off('death_loan_requested');
      socket.off('death_loan_approved'); socket.off('death_loan_forced');
  ...
  const loadCampaign = async () => {
    try {
      ...
      const [npcData, profsData, allPerks, meData, history] = await Promise.all([
        npc.getAll(id).catch(() => []), professions.getAll(), professions.getPerks(),
        auth.me().catch(() => ({})), chat.getMessages(id).catch(() => []),
      ]);
      setNpcs(npcData); setProfessionsList(profsData); setPerks(allPerks);
      ...
      for (const m of (c.members || [])) {
        if (m.character_id) { const ch = await characters.get(m.character_id); chars.push(ch); }
      }
  ...
  const refreshCharacter = async () => {
    if (character?.id) { const updated = await characters.get(character.id); setCharacter(updated); }
  };
  ...
  const sendMessage = async (e) => {
    ...
    if (!text) return;
    setInput('');
    try { await chat.sendMessage(id, text, false); } catch { addMessage({ user: user.username, text, time: new Date().toLocaleTimeString() }); }
  };
  ...
  const handleTimeChange = async (gameTime) => {
    ...
    try { await campaigns.updateTime(id, { game_time: gameTime }); setCampaign(prev => ({ ...prev, game_time: gameTime })); setSaveStatus('saved'); }
    catch { setSaveStatus('error'); }
  };

  const handleKickMember = async (userId) => { await campaigns.deleteMember(id, userId); loadCampaign(); };
  ...
  const tabs = [
    ...
    { key: 'scene', label: 'Сцена' }, { key: 'shop', label: 'Магазин' },
    { key: 'sounds', label: 'Звук' }, { key: 'base', label: 'База' },
    ...(isMaster ? [{ key: 'loot', label: 'Лут' }, { key: 'npcs', label: 'NPC' }, { key: 'notes', label: 'Заметки' }, { key: 'handouts', label: 'Хендауты' }] : [{ key: 'handouts', label: 'Раздача' }]),
    ...(isAdmin ? [{ key: 'admin', label: 'БД' }] : []),
  ];

  return (
    <div className="h-screen bg-wasteland-900 flex flex-col overflow-hidden">
      <CampaignHeader campaign={campaign} navigate={navigate} isMaster={isMaster} hiddenMode={hiddenMode} setHiddenMode={setHiddenMode} saveStatus={saveStatus} onTimeChange={handleTimeChange} />
