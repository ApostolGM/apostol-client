...
export default function Dashboard({ user }) {
  ...
  const create = async (e) => {
    ...
    setError('');
    try {
      const c = await api.createCampaign(title);
  ...
  const join = async (e) => {
    ...
    setError('');
    try {
      const c = await api.joinCampaign(code);
      navigate(`/campaign/${c.id}`);
    } catch (err) {
      setError(err.message);
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-wasteland-800 p-6 rounded-lg border border-wasteland-600">
            ...
            <form onSubmit={create} className="space-y-3">
              <input
                className="w-full bg-wasteland-900 border border-wasteland-600 rounded p-2 text-wasteland-100 placeholder-wasteland-500"
                placeholder="Название кампании"
                value={title}
                onChange={e => setTitle(e.target.value)}
                required
              />
              <button className="w-full bg-wasteland-600 text-wasteland-100 py-2 rounded hover:bg-wasteland-500 transition">
                Создать
              </button>
            </form>
          </div>
            ...
            <div className="space-y-2">
              {campaigns.map(c => (
                <div
                  ...
                  className="bg-wasteland-700 p-3 rounded cursor-pointer hover:bg-wasteland-600 transition flex justify-between items-center"
                >
                  <span>{c.title}</span>
