...
export default function MasterNotes({ campaignId }) {
  ...
  const load = async () => {
    const data = await api.getNotes(campaignId);
    setNotes(data);
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      await api.updateNote(editing, payload);
    } else {
  ...
  const handleEdit = (note) => {
    ...
    setForm({
      ...
      world: note.world || '', region: note.region || '',
      city: note.city || '', location: note.location || '',
  ...
  const resetForm = () => {
    setForm({ title: '', content: '', world: '', region: '', city: '', location: '', tags: '', is_pinned: false });
    setEditing(null);
    setShowForm(false);
  };
  ...
  const filteredNotes = notes.filter(n => {
    ...
    return (n.title?.toLowerCase().includes(f) ||
      ...
      n.world?.toLowerCase().includes(f) ||
  ...
  return (
    <div className="space-y-3">
      ...
      <input
        placeholder="Поиск по тексту, тегам, локации..."
        value={filter}
        onChange={e => setFilter(e.target.value)}
        className="w-full bg-wasteland-800 border border-wasteland-600 rounded p-2 text-wasteland-100 text-sm"
      />

      {/* Форма */}
      {showForm && (
        <div className="bg-wasteland-800 p-4 rounded-lg border border-wasteland-600 space-y-3">
          <h3 className="text-wasteland-300 font-stylized">{editing ? 'Редактировать' : 'Новая заметка'}</h3>
          ...
          <label className="flex items-center gap-2 text-sm text-wasteland-300">
      ...
      {unpinned.length === 0 && pinned.length === 0 && (
        <p className="text-wasteland-500 text-center py-4">Нет заметок</p>
      )}
      {unpinned.map(note => (
        <NoteCard key={note.id} note={note} onEdit={handleEdit} onDelete={handleDelete} />
      ))}
  ...
  return (
    <div className="bg-wasteland-800 p-3 rounded-lg border border-wasteland-600 mb-2">
      <div className="flex justify-between items-start cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div>
          ...
          {(note.tags || []).length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {note.tags.map((tag, i) => (
                <span key={i} className="bg-wasteland-700 text-wasteland-400 text-xs px-1.5 py-0.5 rounded">{tag}</span>
              ))}
            </div>
          )}
        </div>
      ...
      {expanded && note.content && (
        <div className="mt-2 pt-2 border-t border-wasteland-600">
          <p className="text-wasteland-300 text-sm whitespace-pre-wrap">{note.content}</p>
        </div>
      )}
    </div>
  );
