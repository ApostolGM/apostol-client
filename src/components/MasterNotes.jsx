...
export default function MasterNotes({ campaignId }) {
  ...
  const load = async () => { const data = await api.getNotes(campaignId); setNotes(data); setLoading(false); };
  useEffect(() => { load(); }, [campaignId]);

  const handleSave = async () => {
    const tagsArray = form.tags.split(',').map(t => t.trim()).filter(Boolean);
    const payload = { ...form, tags: tagsArray, campaign_id: campaignId };
    if (editing) await api.updateNote(editing, payload);
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => { resetForm(); setShowForm(true); }} className="bg-accent-orange text-wasteland-900 text-sm font-bold px-4 py-2 rounded hover:bg-orange-500 transition">+ Заметка</button>
      </div>
      <input placeholder="Поиск по тексту, тегам, локации..." value={filter} onChange={e => setFilter(e.target.value)} className="w-full bg-wasteland-800 border border-wasteland-600 rounded p-2 text-wasteland-100 text-sm" />
