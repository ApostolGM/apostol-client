...
export default function SoundPad({ campaignId, isMaster, socketRef }) {
  ...
  // Плейлист-режим
  const [playlistMode, setPlaylistMode] = useState(null); // { playlistId, mode: 'sequential'|'random', currentIndex }
  const playlistTimer = useRef(null);

  const audioRef = useRef(null);
  const localAudioRef = useRef(null); // для игроков
  ...
  useEffect(() => {
    ...
    const onPlay = (data) => {
      ...
      if (audio) {
        audio.src = data.url;
        audio.volume = muted ? 0 : data.volume * volume;
  ...
  const handlePlay = (sound) => {
    if (!isMaster) return;
    setPlaying(sound.id);
    setPlaylistMode(null); // ручной выбор
  ...
  // Автопереход при окончании трека (если активен плейлист)
  useEffect(() => {
