...
export default function SoundPad({ campaignId, isMaster, socketRef }) {
  ...
  const load = async () => {
    try {
      const [soundsData, playlistsData] = await Promise.all([api.getSounds(campaignId), api.getPlaylists().catch(() => [])]);
      setSounds(soundsData); setPlaylists(playlistsData);
    } catch (e) { console.error('Ошибка загрузки звуков:', e); }
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const onPlay = (data) => {
      ...
      if (audio) {
        audio.src = data.url; audio.volume = muted ? 0 : data.volume * volume;
        audio.play().catch(() => {});
      }
    };
    const onStop = (data) => {
    ...
    socket.on('sound_play', onPlay); socket.on('sound_stop', onStop);
  ...
  useEffect(() => {
    ...
    const onEnded = () => {
      ...
      let nextIndex;
      if (mode === 'random') nextIndex = Math.floor(Math.random() * tracks.length);
      else { nextIndex = currentIndex + 1; if (nextIndex >= tracks.length) nextIndex = 0; }
      setPlaylistMode(prev => ({ ...prev, currentIndex: nextIndex }));
      const nextTrack = tracks[nextIndex];
      setPlaying(nextTrack.id);
      if (socketRef?.current) socketRef.current.emit('sound_play', { campaignId, soundId: nextTrack.id, url: nextTrack.file_url, volume });
      audio.src = nextTrack.file_url; audio.volume = muted ? 0 : volume; audio.play().catch(() => {});
    };
  ...
  const handleAddSound = async () => {
    ...
    try {
      ...
      setForm({ name: '', file_url: '', category: 'ambient', playlist_id: '' }); setUploadedUrl(''); setShowForm(false);
      load();
  ...
  const handleFileUpload = async (e) => {
    ...
    if (!file) return;
    if (file.size > 40 * 1024 * 1024) { alert('Файл слишком большой. Максимум 40 МБ.'); return; }
    setUploading(true);
    const reader = new FileReader();
    reader.onload = async () => {
      try {
        const result = await api.uploadSound(reader.result, file.name, campaignId, false);
        setForm(prev => ({ ...prev, name: prev.name || result.name, file_url: result.url })); setUploadedUrl(result.url);
      } catch (err) { alert('Ошибка загрузки файла: ' + (err.message || 'Неизвестная ошибка')); }
      finally { setUploading(false); }
    };
    reader.onerror = () => { alert('Ошибка чтения файла'); setUploading(false); };
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {isMaster && (
            <>
              <button onClick={() => { setShowForm(!showForm); setUploadedUrl(''); }} className="bg-accent-orange text-wasteland-900 text-sm font-bold px-3 py-1.5 rounded hover:bg-orange-500 transition">+ Звук</button>
              {playing && <button onClick={handleStop} className="bg-accent-red text-wasteland-900 text-sm font-bold px-3 py-1.5 rounded hover:bg-red-500 transition">⏹ Стоп</button>}
            </>
          )}
        </div>
      </div>
      ...
      {playing && (
        <div className="bg-accent-green/10 border border-accent-green/30 p-2 rounded text-sm text-accent-green flex items-center gap-2">
          <span>🔊</span><span className="truncate">{sounds.find(s => s.id === playing)?.name || 'Неизвестный трек'}</span>
      ...
      {showForm && isMaster && (
        <div className="bg-wasteland-800 p-4 rounded-lg border border-wasteland-600 space-y-3">
          ...
          <input placeholder="Название звука" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-wasteland-900 border border-wasteland-600 rounded p-2 text-wasteland-100 text-sm" />
      ...
      {!isMaster && <audio ref={localAudioRef} preload="auto" />}
            ...
            {playlists.map(pl => (
              <div key={pl.id} className="bg-wasteland-700 rounded px-3 py-1.5 text-xs flex items-center gap-1">
                ...
                {(pl.sounds || []).length > 0 && isMaster && (
                  <>
                    <button onClick={() => startPlaylist(pl, 'sequential')} className="text-accent-green hover:text-green-400 ml-1" title="Играть по порядку">▶</button>
                    <button onClick={() => startPlaylist(pl, 'random')} className="text-accent-yellow hover:text-yellow-400 ml-0.5" title="Играть случайно">🔀</button>
                  </>
                )}
              </div>
            ))}
      ...
      <div className="flex gap-1 overflow-x-auto">
        ...
        {categories.map(cat => (
          <button key={cat} onClick={() => setFilterCategory(cat)} className={`text-xs px-3 py-1 rounded ${filterCategory === cat ? 'bg-accent-orange text-wasteland-900' : 'bg-wasteland-700 text-wasteland-300'}`}>{cat}</button>
        ))}
      </div>

      {Object.keys(soundsByCategory).length === 0 && <p className="text-wasteland-500 text-center py-4">Нет звуков{search ? ' по запросу' : ''}.</p>}
