// components/editors/SkillListEditor.jsx

export default function SkillListEditor({ skills = [], allSkills = [], onChange, showSkillSelect = true, showModifier = true }) {
  const addRow = () => onChange([...skills, { skill: '', modifier: 0 }]);

  const updateRow = (index, field, value) => {
    const updated = skills.map((s, i) => {
      if (i !== index) return s;
      if (field === 'param') return { param: value.replace('param:', ''), modifier: s.modifier || 0 };
  ...
  const getSelectValue = (s) => s.param ? `param:${s.param}` : (s.skill || '');

  return (
    <div className="space-y-2">
      {skills.map((s, i) => (
        <div key={i} className="flex gap-1 items-center">
          <select value={getSelectValue(s)} onChange={e => {
