  ...
  return (
    <div className="bg-wasteland-800 p-4 rounded-lg border border-wasteland-600 space-y-4">
      ...
      <div>
        ...
        {skillList.length === 0 && <p className="text-wasteland-500 text-xs">Нет навыков. Нажмите «+ Навык» чтобы добавить.</p>}
        <div className="space-y-2">
          {skillList.map(skill => (
            <div key={skill.id} className="flex items-center gap-2">
              ...
              <button onClick={() => removeSkill(skill.id)} className="text-accent-red hover:text-red-400 text-sm px-1" title="Удалить навык">✕</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button onClick={handleSubmit} className="bg-accent-orange text-wasteland-900 font-bold px-4 py-2 rounded text-sm hover:bg-orange-500 transition disabled:opacity-50">Сохранить</button>
        <button onClick={onCancel} className="bg-wasteland-600 text-wasteland-300 px-4 py-2 rounded text-sm hover:bg-wasteland-500 transition">Отмена</button>
      </div>
    </div>
  );
}
