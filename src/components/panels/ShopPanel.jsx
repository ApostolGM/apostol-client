// components/panels/ShopPanel.jsx
...
export default function ShopPanel({ character, onRefresh }) {
  ...
  useEffect(() => { setBalance(character?.currency || 0); }, [character]);

  useEffect(() => { shop.getAll().then(setShopItems).catch(e => setError(e.message)).finally(() => setLoading(false)); }, []);

  const handleBuy = async (item) => {
    if (!character) { await alert('Сначала создайте персонажа.'); return; }
    setBuyingId(item.id); setError(''); setSuccessMsg('');
    try {
      const result = await shop.buy(character.id, item.id, 1);
  ...
  return (
    <div className="space-y-4">
      ...
      {error && <div className="bg-accent-red/10 border border-accent-red/30 p-3 rounded text-accent-red text-sm">{error}<button onClick={() => setError('')} className="ml-2 text-wasteland-300">✕</button></div>}
      {successMsg && <div className="bg-accent-green/10 border border-accent-green/30 p-3 rounded text-accent-green text-sm">{successMsg}</div>}
      {!character && <p className="text-wasteland-500 text-sm text-center py-4">Создайте персонажа, чтобы покупать предметы</p>}
      {character && (
      ...
      {filtered.length === 0 && <p className="text-wasteland-500 text-sm text-center py-4">Нет товаров</p>}
        ...
        {filtered.map(item => (
          <div key={item.id} className="bg-wasteland-800 p-3 rounded-lg border border-wasteland-600 flex justify-between items-start">
            <div className="flex-1 min-w-0">
              <h3 className="text-wasteland-100 text-sm font-bold">{item.icon && <span className="mr-1">{item.icon}</span>}{item.name}</h3>
              ...
              {item.description && <p className="text-wasteland-400 text-xs mt-1 line-clamp-2">{item.description}</p>}
              {item.preset_name && <span className="text-wasteland-500 text-xs mt-1 block">📦 {item.preset_name}</span>}
            </div>
            <div className="flex flex-col items-end ml-3 flex-shrink-0">
              <span className="text-accent-orange font-bold text-sm">{item.shop_price || item.trade_price} 💎</span>
              {character && (
                <button onClick={() => handleBuy(item)} disabled={buyingId === item.id} className="mt-1 text-xs bg-accent-orange text-wasteland-900 px-3 py-1 rounded font-bold hover:bg-orange-500 disabled:opacity-50">
