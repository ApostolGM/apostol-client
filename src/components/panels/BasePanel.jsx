// components/panels/BasePanel.jsx
...
export default function BasePanel({ campaignId, character, isMaster, socketRef, onRefresh }) {
  ...
  const load = async () => {
    try {
      const [b, camp] = await Promise.all([
        base.getInventory(campaignId).catch(() => []),
        campaigns.get(campaignId).catch(() => ({})),
      ]);
      setBaseItems(b);
      setBaseAccess(camp?.base_access ?? true);
      if (character?.id) {
        const ch = await characters.get(character.id).catch(() => null);
  ...
  const handleDeposit = async (slotId, qty) => {
    await base.deposit(campaignId, slotId, qty);
    load(); onRefresh?.();
  };

  const handleWithdraw = async (baseItemId, qty) => {
    await base.withdraw(campaignId, baseItemId, qty);
    load(); onRefresh?.();
  };

  const handleToggleAccess = async () => {
    await base.setAccess(campaignId, !baseAccess);
  ...
  return (
    <div className="space-y-4">
      ...
      {!baseAccess && (
        <div className="bg-accent-red/10 border border-accent-red/30 p-3 rounded text-accent-red text-sm text-center">🔒 База недоступна. Вы в рейде.</div>
      )}
