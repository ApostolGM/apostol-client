...
export default function AdminSoundsTab({ sounds, playlists: pls, onRefresh }) {
  ...
  const handleEdit = (s) => { setEditId(s.id); setForm({ name: s.name, category: s.category || 'общее', playlist_id: s.playlist_id || '' }); setShowForm(true); };
  const handleSaveEdit = async () => {
    await request('/admin/sounds/' + editId, { method: 'PUT', body: JSON.stringify(form) });
  ...
  return (
    <div>
      <button onClick={() => { setEditId(null); setForm({ name: '', category: 'ambient', playlist_id: '' }); setShowForm(!showForm); }} className="text-xs bg-accent-orange text-wasteland-900 px-3 py-1.5 rounded mb-3">
        {showForm ? 'Отмена' : '+ Звук'}
      </button>
      ...
      {showForm && (
        <div className="bg-wasteland-800 p-4 rounded-lg border border-wasteland-600 mb-3 space-y-2">
          ...
          <button onClick={editId ? handleSaveEdit : () => {}} disabled={editId ? false : true} className={`w-full py-2 rounded text-sm font-bold ${editId ? 'bg-accent-orange text-wasteland-900' : 'bg-wasteland-700 text-wasteland-500'}`}>
            {editId ? 'Сохранить' : 'Сначала загрузите файл'}
          </button>
        </div>
      )}
        ...
        {sounds.map(s => (
          <div key={s.id} className="bg-wasteland-800 p-2 rounded border border-wasteland-600 text-xs flex justify-between items-center">
            ...
            <div className="flex gap-1">
              <button onClick={() => handleEdit(s)} className="text-wasteland-400 hover:text-wasteland-200">✏</button>
              <button onClick={() => handleDelete(s.id)} className="text-accent-red hover:text-red-400">✕</button>
            </div>
          </div>
        ))}
      </div>
