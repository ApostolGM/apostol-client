...
export default function MasterCharacterCard({ char, expanded, onToggle, onDelete, onParamChange, allSkills, allItems, campaignId, socketRef, onRefresh }) {
  ...
  const handleForceFail = () => {
    if (socketRef?.current) {
      socketRef.current.emit('death_loan_force_fail', { campaignId, characterId: char.id, count: char.death_loan_count });
      onRefresh();
    }
  };

  const handleStatusChange = async (statusId, value) => {
    await charsApi.updateParams(char.id, { statuses: [{ status_id: statusId, value }] });
    onRefresh();
  };
          ...
          <div>
            ...
            {(char.statuses || []).map(s => (
              <div key={s.id} className="flex items-center gap-2 mb-1">
                <span className="text-wasteland-400 text-xs w-20">{s.icon} {s.name}</span>
                <input type="range" min={s.min || 0} max={s.max || 100} value={s.value}
                  onChange={(e) => handleStatusChange(s.id, parseInt(e.target.value))}
                  className="flex-1 h-1.5 rounded cursor-pointer" style={{ accentColor: '#33cc33' }} />
                <span className="text-wasteland-300 text-xs w-8">{s.value}</span>
              </div>
            ))}
          </div>
          ...
          {char.perks?.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {char.perks.map(p => (
                  <span key={p.id} className={`text-xs px-1.5 py-0.5 rounded ${p.cost > 0 ? 'bg-accent-green/20 text-accent-green' : p.cost < 0 ? 'bg-accent-red/20 text-accent-red' : 'bg-wasteland-700 text-wasteland-300'}`}>
                    {p.name} ({p.cost > 0 ? '+' : ''}{p.cost})
                  </span>
                ))}
              </div>
            </div>
