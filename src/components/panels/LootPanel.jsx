// components/panels/LootPanel.jsx
import { useState, useEffect } from 'react';
import { loot } from '../../api/loot.js';
...
export default function LootPanel({ campaignId }) {
  ...
  const load = async () => {
    const [p, i, c] = await Promise.all([
      loot.getPools(campaignId),
      itemsApi.getAll(),
      charsApi.getCampaignCharacters(campaignId).catch(() => []),
    ]);
  ...
  const handleCreate = async () => {
    await loot.createPool(campaignId, poolName, poolItems);
    setPoolName(''); setPoolItems([]); setShowForm(false);
    load();
  };

  const handleDelete = async (id) => {
    await loot.deletePool(id);
    setPools(prev => prev.filter(p => p.id !== id));
  };

  const handleGive = async (poolId, characterId) => {
    setGiving(poolId);
    await loot.giveLoot(poolId, characterId);
    setGiving(null);
    load();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-accent-orange text-wasteland-900 text-sm font-bold px-4 py-2 rounded">{showForm ? 'Отмена' : '+ Лут-пул'}</button>
      </div>
      ...
      {pools.map(pool => (
        <div key={pool.id} className="bg-wasteland-800 p-3 rounded-lg border border-wasteland-600">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {characters.map(ch => (
              <button key={ch.id} onClick={() => handleGive(pool.id, ch.id)} disabled={giving === pool.id} className="text-xs bg-wasteland-700 hover:bg-wasteland-600 px-2 py-1 rounded text-wasteland-300 disabled:opacity-50">
