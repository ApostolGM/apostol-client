// components/character/CharacterCreator.jsx
...
export default function CharacterCreator({ professions, perks, campaignId, onCreated, onCancel }) {
  ...
  const [adaptationTarget, setAdaptationTarget] = useState('');
  ...
  const togglePerk = (perk) => {
    ...
    if (isSelected) {
      ...
      if (perk.name === 'Адаптация') setAdaptationTarget('');
    } else {
  ...
  const handleCreate = async () => {
    ...
    try {
      const perkData = selectedPerks.map(p => {
        const data = { perk_id: p.id };
        if (p.name === 'Адаптация' && adaptationTarget) data.linked_perk_id = adaptationTarget;
        return data;
      });
      const char = await characters.create({
        campaign_id: campaignId, name, profession_id: selectedProf.id,
        perk_ids: selectedPerks.map(p => p.id), perk_data: perkData,
      });
      onCreated(char);
  ...
  const selectedNegativePerks = selectedPerks.filter(p => p.type === 'negative');
  const hasAdaptation = selectedPerks.some(p => p.name === 'Адаптация');
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {step === 1 && (
        <div className="bg-wasteland-800 p-6 rounded-lg border border-wasteland-600 space-y-4">
          ...
          <input className="w-full bg-wasteland-900 border border-wasteland-600 rounded p-3 text-wasteland-100" placeholder="Введите имя..." value={name} onChange={e => setName(e.target.value)} autoFocus />
          ...
          <button onClick={onCancel} className="w-full text-wasteland-400 text-sm hover:text-wasteland-200 py-2">Отмена</button>
        </div>
      )}
      ...
      {step === 3 && selectedProf && (
        <div className="space-y-4">
          ...
          {hasAdaptation && selectedNegativePerks.length > 0 && (
            <div className="bg-wasteland-800 p-4 rounded-lg border border-accent-yellow/30">
              <label className="text-wasteland-300 text-sm block mb-2">Адаптация: какой штраф компенсировать?</label>
              <select value={adaptationTarget} onChange={e => setAdaptationTarget(e.target.value)} className="w-full bg-wasteland-900 border border-wasteland-600 rounded p-2 text-wasteland-100 text-sm">
                <option value="">Выберите негативный перк</option>
                {selectedNegativePerks.map(p => <option key={p.id} value={p.id}>{p.name} ({p.cost > 0 ? '+' : ''}{p.cost})</option>)}
              </select>
            </div>
          )}
