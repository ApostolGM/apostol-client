...
export default function CharacterCreator({ professions, perks, campaignId, onCreated, onCancel }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const perkData = selectedPerks.map(p => {
        ...
        // Если это адаптация и выбран целевой перк
        if (p.name === 'Адаптация' && adaptationTarget) {
          data.linked_perk_id = adaptationTarget;
        }
        return data;
      });
  ...
  // Негативные перки из выбранных (для адаптации)
          ...
          {hasAdaptation && selectedNegativePerks.length > 0 && (
            <div className="bg-wasteland-800 p-4 rounded-lg border border-accent-yellow/30">
              ...
              <select value={adaptationTarget} onChange={e => setAdaptationTarget(e.target.value)} className="w-full bg-wasteland-900 border border-wasteland-600 rounded p-2 text-wasteland-100 text-sm">
                ...
                {selectedNegativePerks.map(p => (
                  <option key={p.id} value={p.id}>{p.name} ({p.cost > 0 ? '+' : ''}{p.cost})</option>
                ))}
              </select>
            </div>
          )}
