...
export default function HandoutsPanel({ campaignId, isMaster }) {
  ...
  const load = async () => { const data = await api.getHandouts(campaignId); setHandouts(data); setLoading(false); };
  useEffect(() => { load(); }, [campaignId]);

  const handleSave = async () => {
    if (editing) await api.updateHandout(editing, form);
    else await api.createHandout({ ...form, campaign_id: campaignId });
    resetForm(); load();
  };
  ...
  const handleDelete = async (id) => {
    if (!await confirm('Удалить хендаут?')) return;
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        ...
        {isMaster && <button onClick={() => { resetForm(); setShowForm(true); }} className="bg-accent-orange text-wasteland-900 text-sm font-bold px-4 py-2 rounded hover:bg-orange-500 transition">+ Хендаут</button>}
      </div>
      ...
      {(isMaster ? handouts : handouts.filter(h => h.is_visible)).length === 0 && <p className="text-wasteland-500 text-center py-4">Нет доступных хендаутов</p>}
          ...
          <div className="flex justify-between items-start">
            ...
            {isMaster && (
              <div className="flex gap-1 flex-shrink-0 ml-2">
                <button onClick={() => handleToggleVisible(h)} className="text-xs px-2 py-1 rounded bg-wasteland-700 text-wasteland-300" title={h.is_visible ? 'Скрыть' : 'Показать'}>{h.is_visible ? '👁' : '👁‍🗨'}</button>
      ...
      {selectedHandout && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setSelectedHandout(null)}>
          <div className="bg-wasteland-800 border border-wasteland-600 rounded-lg p-4 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-3"><h3 className="text-accent-orange font-bold text-lg">{selectedHandout.title}</h3><button onClick={() => setSelectedHandout(null)} className="text-wasteland-400 hover:text-wasteland-200">✕</button></div>
            {selectedHandout.image_url && <img src={selectedHandout.image_url} alt={selectedHandout.title} className="rounded mb-3 max-h-64 w-full object-contain" />}
            {selectedHandout.content && <p className="text-wasteland-200 text-sm whitespace-pre-wrap">{selectedHandout.content}</p>}
